...
export class ReportsService extends BaseService<Report> {
  ...
  async list(dto: QueryReportsDto): Promise<IPagination<Report>> {
    ...
    if (isAutoGenerated !== undefined)
      whereCondition.isAutoGenerated = isAutoGenerated;
    ...
    // Build relations
    const relations = ['user', 'moderator', 'actions', 'actions.moderator'];
  ...
  async getStats(dto: ReportStatsDto): Promise<{
    ...
    const reportsByStatus = await this.getReportsByField(
      'status',
      whereCondition,
    );
    const reportsByPriority = await this.getReportsByField(
      'priority',
      whereCondition,
    );
    const reportsByType = await this.getReportsByField(
      'reportableType',
      whereCondition,
    );
    const reportsByReason = await this.getReportsByField(
      'reason',
      whereCondition,
    );
    ...
    const averageResolutionTime =
      resolvedReportsWithTimes.length > 0
    ...
    // Get top users
    const topUsers = await this.getTopUsers(whereCondition);
